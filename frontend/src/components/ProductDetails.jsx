...
const ProductReviews = ({ product, userEmail, handleReviewSubmit, rating, setRating, comment, setComment }) => (
            ...
            <div className="write-review">
                ...
                <form onSubmit={handleReviewSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '1rem' }}>
                    <div className="form-group">
                        ...
                        <select id="rating" value={rating} onChange={(e) => setRating(e.target.value)} className="form-input" style={{ width: '100px' }}>
...
const RecentProducts = ({ recentProducts, getImageUrl }) => {
        ...
        <div style={{ marginTop: '3rem', padding: '2rem', background: 'var(--surface)', borderRadius: 'var(--radius-lg)' }}>
            ...
            <div style={{ display: 'flex', gap: '1.5rem', overflowX: 'auto', paddingBottom: '1rem' }}>
                {recentProducts.map(prod => (
                    <div key={prod._id} role="button" tabIndex="0" onKeyDown={(e) => { if(e.key === 'Enter' || e.key === ' ') window.location.href = `/product/${prod._id}`; }} style={{ minWidth: '200px', width: '200px', background: 'var(--card-bg)', borderRadius: 'var(--radius-md)', padding: '1rem', border: '1px solid var(--border)', cursor: 'pointer' }} onClick={() => window.location.href = `/product/${prod._id}`}>
                        <img
            ...
            <div className="product-details-page">
                ...
                <div className="product-details-info">
                    ...
                    <div className="quantity-selector">
                        <label htmlFor="quantity" style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', fontWeight: '500' }}>Quantity:</label>
                        <input id="quantity" type="number" value={quantity} onChange={(e) => setQuantity(e.target.value)} min="1" max={product.stock} className="quantity-input" />
                    </div>
